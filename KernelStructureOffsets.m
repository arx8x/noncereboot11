...
int koffset(enum kstruct_offset offset)
{
    ...
    dispatch_once(&onceToken, ^{
        DEBUG("kCFCoreFoundationVersionNumber: %f", kCFCoreFoundationVersionNumber);
        if (kCFCoreFoundationVersionNumber >= 1535.12) {
            DEBUG("offsets selected for iOS 12.0 or above");
            offsets = kstruct_offsets_12_0;
        } else if (kCFCoreFoundationVersionNumber >= 1452.23) {
            DEBUG("offsets selected for iOS 11.3 or above");
            offsets = kstruct_offsets_11_3;
        } else if (kCFCoreFoundationVersionNumber >= 1443.00) {
            DEBUG("offsets selected for iOS 11.0 to 11.2.6");
            offsets = kstruct_offsets_11_0;
        } else {
            DEBUG("iOS version too low, 11.0 required");
            exit(EXIT_FAILURE);
        }
    });
    if (offsets == NULL) {
