...
size_t kread(uint64_t where, void* p, size_t size)
{
    ...
    while (offset < size) {
        ...
        rv = mach_vm_read_overwrite(tfp0,
                                    ...
                                    &sz);
        if (rv || sz == 0) {
            DEBUG("error reading kernel @%p", (void*)(offset + where));
            break;
        }
        offset += sz;
    }
...
size_t kwrite(uint64_t where, const void* p, size_t size)
{
    ...
    while (offset < size) {
        ...
        if (rv) {
            DEBUG("error writing kernel @%p", (void*)(offset + where));
            break;
        }
        offset += chunk;
    }
...
bool wkbuffer(uint64_t kaddr, void* buffer, size_t length)
{
    if (tfp0 == MACH_PORT_NULL) {
        DEBUG("attempt to write to kernel memory before any kernel memory write primitives available");
        return false;
    }
...
uint32_t rk32_via_kmem_read_port(uint64_t kaddr)
{
    ...
    if (kmem_read_port == MACH_PORT_NULL) {
        DEBUG("kmem_read_port not set, have you called prepare_rk?");
        exit(EXIT_FAILURE);
    }
    ...
    if (err != KERN_SUCCESS) {
        DEBUG("error setting context off of dangling port: %x %s", err, mach_error_string(err));
        exit(EXIT_FAILURE);
    }
    ...
    if (err != KERN_SUCCESS) {
        DEBUG("error calling pid_for_task %x %s", err, mach_error_string(err));
        exit(EXIT_FAILURE);
    }
...
uint32_t ReadKernel32(uint64_t kaddr)
{
    ...
    DEBUG("attempt to read kernel memory but no kernel memory read primitives available");
...
{
    if (tfp0 == MACH_PORT_NULL) {
        DEBUG("attempt to allocate kernel memory before any kernel memory write primitives available");
        return 0;
    }
    ...
    if (err != KERN_SUCCESS) {
        DEBUG("unable to allocate kernel memory via tfp0: %s %x", mach_error_string(err), err);
        return 0;
    }
    return addr;
}
...
uint64_t kmem_alloc_wired(uint64_t size)
{
    ...
    DEBUG("vm_kernel_page_size: %lx", vm_kernel_page_size);
    ...
    DEBUG("allocated address: %llx", addr);
    
    addr += 0x3fff;
    addr &= ~0x3fffull;
    
    DEBUG("address to wire: %llx", addr);
    
    err = mach_vm_wire(fake_host_priv(), tfp0, addr, ksize, VM_PROT_READ | VM_PROT_WRITE);
    if (err != KERN_SUCCESS) {
        DEBUG("unable to wire kernel memory via tfp0: %s %x", mach_error_string(err), err);
        return 0;
    }
    return addr;
}
...
{
    if (tfp0 == MACH_PORT_NULL) {
        DEBUG("attempt to deallocate kernel memory before any kernel memory write primitives available");
        return false;
    }
    ...
    if (err != KERN_SUCCESS) {
        DEBUG("unable to deallocate kernel memory via tfp0: %s %x", mach_error_string(err), err);
        return false;
    }
    return true;
}
...
{
    if (tfp0 == MACH_PORT_NULL) {
        DEBUG("attempt to change protection of kernel memory before any kernel memory write primitives available");
        return false;
    }
    kern_return_t err;
    err = mach_vm_protect(tfp0, (mach_vm_address_t)kaddr, (mach_vm_size_t)size, 0, (vm_prot_t)prot);
    if (err != KERN_SUCCESS) {
        DEBUG("unable to change protection of kernel memory via tfp0: %s %x", mach_error_string(err), err);
        return false;
    }
    return true;
}
